...
describe('Modules', () => {
  ...
  it('Create module', (done) => {
    objectModule.addModule(moduleData, db).then((inserted) => {
  ...
  it('Getting module', (done) => {
    objectModule.getModule(moduleData._id, db).then((fetched) => {
  ...
  it('Getting module list', (done) => {
    objectModule.getModules(db).then((fetched) => {
  ...
  it('Adding fields to module', async () => {
    const result = await objectModule.addModuleFields(moduleData._id, moduleFieldData, db)
      .then(r => r.ok)
      .catch(err => (err));
    assert.equal(result, true);
    moduleData.fields = moduleFieldData;
    return objectModule.getModule(moduleData._id, db).then((fetched) => {
      const keys = Object.keys(moduleData);
      keys.forEach((key) => {
        if (key === 'fields') {
          fetched[key].forEach((f, i) => assert.equal(f.displayName, moduleData[key][i].displayName));
        } else {
  ...
  it('Remove fields from module', async () => {
    ...
    const result = await objectModule.removeModuleFields(moduleData._id, [poppedField.name], db)
      .then(r => r.ok)
      .catch(err => (err));
    assert.equal(result, true);
    moduleData.fields = moduleFieldData;
  ...
  it('Updating module', async () => {
    ...
    return objectModule.updateModule(moduleData._id, { name }, db).then((fetched) => {
      assert.equal(fetched, true);
      return objectModule.getModule(moduleData._id, db).then((fetchedAgain) => {
        assert.equal(fetchedAgain.name, name);
      });
    });
  });
  it('Removing module', (done) => {
    objectModule.removeModule(moduleData._id, db)
      .then((fetched) => {
        assert.equal(fetched, true);
        done();
      });
